...
public class SirenOrderHomeViewHandler {
    ...
    public void whenSaved_then_CREATE_1(@Payload Saved saved) {
        try {
            if (saved.isMe()) {
                List<SirenOrderHome> sirenOrderHomeList = sirenOrderHomeRepository.findByOrderId(saved.getOrderId());
                for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                    sirenOrderHome.setShopId(saved.getId());
                    sirenOrderHome.setOrderId(saved.getOrderId());
                    sirenOrderHome.setPoint(saved.getPoint());
                    sirenOrderHome.setStatus("Milage Saved");
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    // view 레파지 토리에 save
                    sirenOrderHomeRepository.save(sirenOrderHome);
                }
    ...
    public void whenCanceled_then_DELETE_1(@Payload Canceled canceled) {
        try {
            if (canceled.isMe()) {
                List<SirenOrderHome> sirenOrderHomeList = sirenOrderHomeRepository.findByOrderId(canceled.getOrderId());
                for(SirenOrderHome sirenOrderHome : sirenOrderHomeList){
                    sirenOrderHome.setShopId(canceled.getId());
                    sirenOrderHome.setOrderId(canceled.getOrderId());
                    sirenOrderHome.setPoint(canceled.getPoint());
                    sirenOrderHome.setStatus("Milage Canceled!");
                    // view 객체에 이벤트의 eventDirectValue 를 set 함
                    // view 레파지 토리에 save
                    sirenOrderHomeRepository.save(sirenOrderHome);
                }
